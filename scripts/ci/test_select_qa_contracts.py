...
class QAContractSelectorTests(unittest.TestCase):
    ...
    def test_transport_change_selects_all_runtime_contracts(self):
        selected, _ = SELECTOR.select(
            MANIFEST, ["internal/agentcore/run.go"], False
        )
        self.assertEqual(selected, sorted(MANIFEST["contracts"]))

    def test_platform_change_selects_cross_platform_and_container(self):
        selected, _ = SELECTOR.select(
            MANIFEST, ["internal/agentplatform/windows/collect.go"], False
        )
        self.assertIn("agent-cross-platform", selected)
        self.assertIn("agent-installed-container", selected)
    ...
    def test_unmapped_high_risk_path_fails_closed(self):
        ...
        with self.assertRaisesRegex(ValueError, "no QA rule"):
            SELECTOR.select(manifest, ["secure/new-risk/file.go"], False)
