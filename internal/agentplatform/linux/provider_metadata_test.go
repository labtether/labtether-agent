...
func TestReadTailscaleMetadataWithStatusAndVersionJSON(t *testing.T) {
		...
		func(_ time.Duration, _ string, args ...string) ([]byte, error) {
			if len(args) >= 2 && args[0] == "status" && args[1] == "--json" {
				return []byte(`{
					...
					"Self":{"DNSName":"lab-host.labnet.ts.net","TailscaleIPs":["100.64.0.2","fd7a:115c:a1e0::2"],"ExitNode":false}
				}`), nil
			}
	...
	if got := metadata["tailscale_self_dns_name"]; got != "lab-host.labnet.ts.net" {
		t.Fatalf("tailscale_self_dns_name=%q, want lab-host.labnet.ts.net", got)
	}
	if got := metadata["tailscale_self_tailscale_ip"]; got != "100.64.0.2,fd7a:115c:a1e0::2" {
...
func TestReadCapabilityMetadataWithToolingPresent(t *testing.T) {
	metadata := readCapabilityMetadataWith(func(name string) (string, error) {
		switch name {
		case "systemctl", "apt-get", "journalctl", "netplan":
	...
	if got := metadata["service_backend"]; got != "systemd" {
		t.Fatalf("service_backend=%q, want systemd", got)
	}
	if got := metadata["cap_packages"]; got != "list,action" {
		t.Fatalf("cap_packages=%q, want list,action", got)
	}
	if got := metadata["package_backend"]; got != "apt-get" {
		t.Fatalf("package_backend=%q, want apt-get", got)
	}
	...
	if got := metadata["network_action_backend"]; got != "netplan" {
		t.Fatalf("network_action_backend=%q, want netplan", got)
	}
}

func TestReadCapabilityMetadataWithoutTooling(t *testing.T) {
